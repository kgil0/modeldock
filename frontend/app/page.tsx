...
export default function Home() {
  ...
  async function sendPrompt() {
    ...
    try {
      const createRes = await fetch("/api/tasks/chat", {
        method: "POST",
        headers: {
      ...
      for (let i = 0; i < 120; i++) {
        ...
        if (task.status === "finished") {
          finalResponse = task.result?.response || JSON.stringify(task.result);
          break;
        }
      ...
      setResponse(finalResponse || "Task timeout.");
    } catch {
...
if (!loggedIn) {
  ...
  return (
    <main className="min-h-screen bg-black text-white p-10">
      ...
      <section className="bg-zinc-950 border border-zinc-800 p-6 rounded-xl mb-8">
        <h2 className="text-2xl mb-4">Model Store</h2>

        <div className="grid grid-cols-1 gap-4">
          {catalog.map((model: any) => {
            const installed = nodes
              .find((n: any) => n.id === selectedNode)
              ?.models?.some((m: string) => m.startsWith(model.name));
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="text-sm text-zinc-500">{model.name}</p>
                    <p className="text-sm text-zinc-400 mt-2">{model.description}</p>
                    ...
                    <div className="text-sm text-zinc-400 mt-3">
                      Author: {model.author} . Size: {model.size} . RAM: {model.ram}
                    </div>
                  </div>
